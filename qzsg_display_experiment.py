...
file_path = 'experiment_1_qubits_50_experiments.pkl'
experiments = read_all_outputs(file_path)

payoff_obs_exp_vals = []
mmwu_dual_gap_exp_vals = []
mmwu_iter_times_exp_vals = []
mmwu_sd_dual_gap_exp_vals = []
mmwu_sd_iter_times_exp_vals = []
ommwu_dual_gap_exp_vals = [] 
ommwu_iter_time_exp_vals = []

for exp_num, exp_outcome in enumerate(experiments):
    payoff_obs, iters_save, mmwu_dual_gaps, mmwu_iter_times, mmwu_sd_dual_gaps, mmwu_sd_iter_times, ommwu_dual_gaps, ommwu_iter_times = exp_outcome
    ...
    mmwu_sd_dual_gap_exp_vals.append(np.abs(mmwu_sd_dual_gaps))
    mmwu_sd_iter_times_exp_vals.append(mmwu_sd_iter_times)
...
plt.loglog(iters_save, mean_mmwu_dual_gap, color='tab:orange',label='MMWU Mean')
...
# Plot MMWU-SD mean and confidence interval
mean_mmwu_sd_dual_gap = np.mean(mmwu_sd_dual_gap_2d, axis=0)
std_mmwu_sd_dual_gap = np.std(mmwu_sd_dual_gap_2d, axis=0)
n_mmwu = mmwu_sd_dual_gap_2d.shape[0]
mmwu_sd_confidence_interval = 1.96 * std_mmwu_sd_dual_gap / np.sqrt(n_mmwu)  # 95% confidence interval

plt.loglog(iters_save, mean_mmwu_sd_dual_gap, color='tab:pink',label='MMWU-SD Mean')
plt.fill_between(iters_save, mean_mmwu_sd_dual_gap - mmwu_sd_confidence_interval, mean_mmwu_sd_dual_gap + mmwu_sd_confidence_interval, color='tab:pink', alpha=0.2, label='MMWU-SD 95% Confidence Interval')
...
plt.loglog(iters_save, mean_ommwu_dual_gap, color='tab:blue',label='OMMWU Mean')
...
for result in mmwu_sd_dual_gap_exp_vals:
    plt.loglog(iters_save, result, color='tab:pink', alpha=0.1)  # Plot each experiment with low opacity
...
mmwu_sd_runtime_2d = np.vstack(mmwu_sd_iter_times_exp_vals)
...
plt.figure(figsize=(10, 6))
plt.plot(iters_save_time, mean_mmwu_runtime, color='tab:orange',label='MMWU Mean')
...
# Plot MMWU-SD mean and confidence interval
mean_mmwu_sd_runtime = np.mean(mmwu_sd_runtime_2d, axis=0)
std_mmwu_sd_runtime = np.std(mmwu_sd_runtime_2d, axis=0)
n_mmwu = mmwu_runtime_2d.shape[0]
mmwu_sd_conf_int_runtime = 1.96 * std_mmwu_sd_runtime / np.sqrt(n_mmwu)  # 95% confidence interval

plt.plot(iters_save_time, mean_mmwu_sd_runtime, color='tab:pink',label='MMWU-SD Mean')
plt.fill_between(iters_save_time, mean_mmwu_sd_runtime - mmwu_sd_conf_int_runtime, mean_mmwu_sd_runtime + mmwu_sd_conf_int_runtime, color='tab:pink', alpha=0.2, label='MMWU-SD 95% Confidence Interval')
...
plt.plot(iters_save_time, mean_ommwu_runtime, color='tab:blue',label='OMMWU Mean')
